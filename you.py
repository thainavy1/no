...
#=============
#cl = LineClient()
cl = LineClient(authToken='EAcwOrWcyISyIlgg5qLa.a6dzd0TpLsSRAJiksGPbgG.5qA9ygW6GVmvVZYG9CFvXyWqa/50d2hbZlNaI7oKLQ0=')
...
#sw = LineClient()
sw = LineClient(authToken='EARzunuodfcW8CcrqcM0.3Ue5K3qM3D42m7t4fN5kma.E0kdwZ0M9G5pHzgLO1hPr3qC/Xtprc06NoboHbGmFqI=')
                        ...
                        if cmd == "selfbot on":
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                if wait["autoLeave"] == True: md+="ãð³ã AutoleaveãONã\n"
                                else: md+="ãð´ã AutoleaveãOFFã\n"
                                if msg.to in protectqr: md+="ãð³ã ProtecturlãONã\n"
                                else: md+="ãð´ã ProtecturlãOFFã\n"
                                if msg.to in protectjoin: md+="ãð³ã ProtectjoinãONã\n"
                                else: md+="ãð´ã ProtectjoinãOFFã\n"
                                if msg.to in protectkick: md+="ãð³ã ProtectkickãONã\n"
                                else: md+="ãð´ã ProtectkickãOFFã\n"
                                if msg.to in protectinvite: md+="ãð³ã ProtectinviteãONã\n"
                                else: md+="ãð´ã ProtectinviteãOFFã\n"
                                if msg.to in protectcancel: md+="ãð³ã ProtectcancelãONã\n"
                                else: md+="ãð´ã ProtectcancelãOFFã\n"
                                if msg.to in protectantijs: md+="ãð³ã ProtectantijsãONã\n"
                                else: md+="ãð´ã Protectantijs ãOFFã\n"  
                                if msg.to in ghost: md+="ãð³ã Ghost ãONã\n"
                                else: md+="ãð´ã Ghost ãOFFã\n"
                            ...
                            if msg._from in admin:
                                cl.sendText(msg.to,"ãCreated by :  Oz.âï¸ ã") 
                                ma = ""
                                for i in creator:
                                    ma = cl.getContact(i)
                          ...
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                            try:
                                ...
                                bot = runtime(eltime)
                                start = time.time()
                                sw.sendText("u1249c600ed5713961da8b8c0bf9ba3ff", ':)')
                                elapsed_time = time.time() - start
                                ryan = cl.getContact(mid)
                                zx = ""
                                zxc = ""
                                zx2 = []
                                xpesan =  "ã Informasi Selfbot ã\nâ¢ User : "
                                ret_ = "â¢ Group : {} Group".format(str(len(grouplist)))
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               sendMention(msg.to, sender, "ã User Selfbot ã\n", "")
                               msg.contentType = 13
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               sendMention(msg.to, sender, "ã Restarting ã\nUser ", "\nTunggu sebentar...")
...
#===========COMMAND ON OFF============#
                        ...
                        elif cmd == "เปิดบล็อก" or text.lower() == 'autoblock on':
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                wait["autoBlock"] = True
                                cl.sendText(msg.to,"ã Status Autoleave ã\nAutoleave telah diaktifkan")

                        elif cmd == "ปิดบล็อก" or text.lower() == 'autoblock off':
                          if wait["selfbot"] == True:
                            if msg._from in admin:
